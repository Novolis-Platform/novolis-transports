...
using Novolis.Transports.Torrent.Exceptions;
...
public sealed class TorrentClient : IDisposable
{
    ...
    /// <summary>
    ///     Initializes a new instance of the <see cref="Novolis.Transports.Torrent" /> class.
    /// </summary>
    ...
    /// <summary>
    ///     Prevents a default instance of the <see cref="Novolis.Transports.Torrent" /> class from being created.
    /// </summary>
    ...
    public void Dispose()
    {
        ...
        {
            Debug.WriteLine($"dispose stop: {ex.Message}");
        }
    ...
    public TorrentProgressInfo GetProgressInfo(string torrentInfoHash)
    {
        ...
        if (this.transfers.TryGetValue(torrentInfoHash, out transferManager))
            return new TorrentProgressInfo(
                ...
                transferManager.CompletedPercentage * 100m,
    ...
    /// <summary>
    ///     Adds an outbound peer connection for an active torrent (dogfood / local seed harness).
    /// </summary>
    public void AddPeer(string torrentInfoHash, IPEndPoint endpoint)
    {
        ...
        lock (((IDictionary)this.transfers).SyncRoot)
        {
            if (!this.transfers.TryGetValue(torrentInfoHash, out var transfer))
    ...
    public void Start()
    {
        ...
        // start listening
        this.server = new TcpListener(IPAddress.Any, this.ListeningPort);
        this.server.Start();
    ...
    public void Start(TorrentInfo torrentInfo)
    {
        ...
        if (this.IsRunning)
            lock (((IDictionary)this.transfers).SyncRoot)
            {
                ...
                {
                    transfer = new TransferManager(this.ListeningPort, torrentInfo, this.throttlingManager,
                        new PersistenceManager(this.BaseDirectory, torrentInfo.Length, torrentInfo.PieceLength,
                            torrentInfo.PieceHashes, torrentInfo.Files));
                        ...
                        if (message is HandshakeMessage)
                            lock (((IDictionary)this.transfers).SyncRoot)
                            {
                                if (this.transfers.TryGetValue(message.As<HandshakeMessage>().InfoHash, out transfer))
                                    ...
                                    Debug.WriteLine(
                                        $"invalid torrent info hash: {message.As<HandshakeMessage>().InfoHash} received");
                            }
            }
