...
public class BEncodedList : BEncodedValue, IList<BEncodedValue>, IEnumerable
{
    ...
    public override int Encode(byte[] buffer, int offset)
    {
        ...
        for (var i = 0; i < this.list.Count; i++) written += this.list[i].Encode(buffer, offset + written);
    ...
    public override int GetHashCode()
    {
        ...
        for (var i = 0; i < this.list.Count; i++) result ^= this.list[i].GetHashCode();
    ...
    public override int LengthInBytes()
    {
        ...
        for (var i = 0; i < this.list.Count; i++) length += this.list[i].LengthInBytes();
    ...
    internal override void DecodeInternal(RawReader reader)
    {
        ...
        if (reader.ReadByte() != 'l') throw new BEncodingException("Invalid data found. Aborting");

        while (reader.PeekByte() != -1 &&
               reader.PeekByte() != 'e')
            this.list.Add(Decode(reader));

        if (reader.ReadByte() != 'e') throw new BEncodingException("Invalid data found. Aborting");
    }
}
