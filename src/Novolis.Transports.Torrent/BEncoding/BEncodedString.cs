...
namespace Novolis.Transports.Torrent.BEncoding;
...
public class BEncodedString : BEncodedValue, IComparable<BEncodedString>
{
    ...
    public string Hex
    {
        get { return BitConverter.ToString(this.TextBytes); }
    }
    ...
    public string Text
    {
        get { return Encoding.ASCII.GetString(this.textBytes); }

        set { this.textBytes = Encoding.ASCII.GetBytes(value); }
    }
    ...
    public override int GetHashCode()
    {
        ...
        for (var i = 0; i < this.textBytes.Length; i++) hash += this.textBytes[i];
    ...
    public override int LengthInBytes()
    {
        ...
        for (var i = this.textBytes.Length; i != 0; i = i / 10) prefix += 1;

        if (this.textBytes.Length == 0) prefix++;
    ...
    public int WriteAscii(byte[] buffer, int offset, string text)
    {
        ...
        for (var i = 0; i < text.Length; i++) this.Write(buffer, offset + i, (byte)text[i]);
    ...
    internal override void DecodeInternal(RawReader reader)
    {
        ...
        if (reader.ReadByte() != ':') throw new BEncodingException("Invalid data found. Aborting");
