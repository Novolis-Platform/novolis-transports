...
public class AnnounceResponseMessage
{
    ...
    private AnnounceResponseMessage(string failureReason, TimeSpan updateInterval, int seedersCount, int leecherCount,
        ...
        if (data.IsNotNullOrEmpty())
        {
            ...
            if (value is BEncodedDictionary)
            {
                ...
                if (value.As<BEncodedDictionary>().ContainsKey(intervalKey) &&
                    ...
                    interval = TimeSpan.FromSeconds(value.As<BEncodedDictionary>()[intervalKey].As<BEncodedNumber>()
                        .Number);
                else
                    return false;
                ...
                if (value.As<BEncodedDictionary>().ContainsKey(peersKey) &&
                    ...
                    foreach (var item in value.As<BEncodedDictionary>()[peersKey].As<BEncodedList>())
                        if (item is BEncodedDictionary)
                        {
                            ...
                            {
                                peerId = Message.ToPeerId(Encoding.ASCII.GetBytes(
                                    item.As<BEncodedDictionary>()[peerIdKey].As<BEncodedString>().Text));
                        ...
                        else
                        {
                            ...
                            for (var i = 0; i < data2.Length; i += 6)
                            {
                                endpoint = new IPEndPoint(
                                    new IPAddress(IPAddress.NetworkToHostOrder(BitConverter.ToInt32(data2, i))),
                                    IPAddress.NetworkToHostOrder(BitConverter.ToInt16(data2, i + 4)));

                                if (!peers.ContainsKey(endpoint.ToString())) peers.Add(endpoint.ToString(), endpoint);
                            }
                        }
                else
                    return false;
