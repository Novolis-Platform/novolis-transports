...
namespace Novolis.Transports.Torrent.TrackerProtocol.Udp.Messages;
...
public class AnnounceResponseMessage : TrackerMessage
{
    ...
    public AnnounceResponseMessage(int transactionId, TimeSpan interval, int leecherCount, int seederCount,
    ...
    public override int Length
    {
        get
        {
            return ActionLength + TransactionIdLength + IntervalLength + LeechersLength + SeedersLength +
                   this.Peers.Count() * (IpAddressLength + PortLength);
        }
    }
    ...
    public static bool TryDecode(byte[] buffer, int offset, out AnnounceResponseMessage message)
    {
        ...
        if (buffer != null &&
            buffer.Length >= offset + ActionLength + TransactionIdLength + IntervalLength + LeechersLength +
            SeedersLength &&
            offset >= 0)
        {
                ...
                seeders >= 0)
            {
                ...
                message = new AnnounceResponseMessage(transactionId, TimeSpan.FromSeconds(interval), leechers, seeders,
                    peers.Values);
            }
        }
    ...
    public override int Encode(byte[] buffer, int offset)
    {
        ...
        Write(buffer, ref written, (int)this.Interval.TotalSeconds);
        Write(buffer, ref written, this.LeecherCount);
        Write(buffer, ref written, this.SeederCount);

        foreach (var peerEndpoint in this.Peers) Write(buffer, ref written, peerEndpoint);
