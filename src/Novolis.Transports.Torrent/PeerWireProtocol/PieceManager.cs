...
namespace Novolis.Transports.Torrent.PeerWireProtocol;
...
public sealed class PieceManager : IDisposable
{
    ...
    public PieceManager(string torrentInfoHash, long torrentLength, IEnumerable<string> pieceHashes, long pieceLength,
        int blockLength, PieceStatus[] bitField)
    {
        ...
        {
            if (this.BitField[i] != PieceStatus.Ignore) this.piecesCount++;

            if (bitField[i] == PieceStatus.Present) this.presentPiecesCount++;
        }

        this.CompletedPercentage = this.piecesCount == 0
            ? 0
            : this.presentPiecesCount / (decimal)this.piecesCount;
    ...
    public Piece CheckOut(int pieceIndex, byte[] pieceData = null, bool[] bitField = null)
    {
        ...
        if (pieceData != null) Array.Clear(pieceData, 0, pieceData.Length);

        if (bitField != null) Array.Clear(bitField, 0, bitField.Length);
        ...
        lock (this.locker)
        {
                 ...
                 this.IsEndGame))
            {
                ...
                if (!this.checkouts.ContainsKey(pieceIndex)) this.checkouts.Add(pieceIndex, DateTime.UtcNow);
            }
        }
    ...
    private void OnPieceCompleted(object sender, PieceCompletedEventArgs e)
    {
        ...
        if (this.PieceCompleted != null) this.PieceCompleted(sender, e);
    }
    ...
    private void OnPieceRequested(object sender, PieceRequestedEventArgs e)
    {
        ...
        if (this.PieceRequested != null) this.PieceRequested(sender, e);
    }
    ...
    private void Piece_Completed(object sender, PieceCompletedEventArgs e)
    {
        lock (this.locker)
        {
                ...
                this.BitField[e.PieceIndex] == PieceStatus.CheckedOut)
            {
                ...
                this.CompletedPercentage = this.presentPiecesCount / (decimal)this.piecesCount;

                if (this.checkouts.ContainsKey(e.PieceIndex)) this.checkouts.Remove(e.PieceIndex);
    ...
    private void Timer_Elapsed(object sender, ElapsedEventArgs e)
    {
        ...
        lock (this.locker)
        {
            foreach (var checkOut in this.checkouts)
            {
                ...
                if (DateTime.UtcNow - checkoutTime > this.checkoutTimeout) checkoutsToRemove.Add(checkOut.Key);
            }
