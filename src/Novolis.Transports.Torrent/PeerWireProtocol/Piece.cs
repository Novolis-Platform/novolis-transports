...
public sealed class Piece
{
    ...
    public Piece(int pieceIndex, string pieceHash, long pieceLength, int blockLength, int blockCount,
        byte[] pieceData = null, bool[] bitField = null)
    {
    ...
    public void PutBlock(int blockOffset, byte[] blockData = null)
    {
        ...
        var blockIndex = this.GetBlockIndex(blockOffset);

        if (!this.BitField[blockIndex])
        {
            this.BitField[blockIndex] = true;

            if (blockData != null) Buffer.BlockCopy(blockData, 0, this.PieceData, blockOffset, blockData.Length);

            this.completedBlockCount++;

            if (this.completedBlockCount == this.BlockCount)
            {
                if (string.Compare(this.PieceData.CalculateSha1Hash(0, (int)this.PieceLength).ToHexaDecimalString(),
                        this.PieceHash, true, CultureInfo.InvariantCulture) == 0)
                {
    ...
    private void OnCompleted(object sender, PieceCompletedEventArgs e)
    {
        ...
        if (this.Completed != null) this.Completed(sender, e);
    }
    ...
    private void OnCorrupted(object sender, EventArgs e)
    {
        ...
        if (this.Corrupted != null) this.Corrupted(sender, e);
    }
}
