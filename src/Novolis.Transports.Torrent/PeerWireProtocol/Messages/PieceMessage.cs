...
public class PieceMessage : PeerMessage
{
    ...
    /// </returns>
    public static bool TryDecode(byte[] buffer, ref int offsetFrom, int offsetTo, out PieceMessage message,
        out bool isIncomplete, byte[] destination = null)
    {
        int messageLength;
        byte messageId;
        int pieceIndex;
        ...
        if (buffer != null &&
            buffer.Length > offsetFrom2 + MessageLengthLength + MessageIdLength + PieiceIndexLength +
            BlockOffsetLength &&
            offsetFrom2 >= 0 &&
    ...
    public override int Encode(byte[] buffer, int offset)
    {
        ...
        Write(buffer, ref written, MessageIdLength + PieiceIndexLength + BlockOffsetLength + this.Data.Length);
        Write(buffer, ref written, MessageId);
        Write(buffer, ref written, this.PieceIndex);
        Write(buffer, ref written, this.BlockOffset);
        Write(buffer, ref written, this.Data);
    ...
    public override bool Equals(object obj)
    {
        ...
        if (msg == null)
            return false;
        if (this.PieceIndex == msg.PieceIndex &&
    ...
    {
        return
            $"PieceMessage: PieceIndex = {this.PieceIndex}, BlockOffset = {this.BlockOffset}, BlockData = byte[{this.Data.Length}]";
    }
}
