...
namespace Novolis.Transports.Torrent.PeerWireProtocol.Messages;
...
public class HandshakeMessage : PeerMessage
{
    ...
    /// <exception cref="Novolis.Transports.Torrent.Exceptions.PeerWireProtocolException">
    ...
    /// </exception>
    public HandshakeMessage(string infoHash, string peerId, string protocolString = ProtocolName,
        bool supportsFastPeer = false, bool supportsExtendedMessaging = false)
    {
    ...
    {
        get { return NameLengthLength + this.ProtocolString.Length + ReservedLength + InfoHashLength + PeerIdLength; }
    }
    ...
    /// </returns>
    public static bool TryDecode(byte[] buffer, ref int offsetFrom, int offsetTo, out HandshakeMessage message,
            ...
            if (buffer.Length >= offsetFrom + protocolStringLength + ReservedLength + InfoHashLength + PeerIdLength)
            {
                    ...
                    peerId.IsNotNullOrEmpty())
                {
                    if (offsetFrom <= offsetTo)
                        message = new HandshakeMessage(infoHash, peerId, protocolString, supportsFastPeer,
    ...
    public override int Encode(byte[] buffer, int offset)
    {
        ...
        Write(buffer, ref written, (byte)this.ProtocolString.Length);
        Write(buffer, ref written, this.ProtocolString);
        Write(buffer, ref written, ZeroedBits);

        if (this.SupportsExtendedMessaging) buffer[written - 3] |= ExtendedMessagingFlag;

        if (this.SupportsFastPeer) buffer[written - 1] |= FastPeersFlag;
