...
public class BitFieldMessage : PeerMessage
{
    ...
    public BitFieldMessage(bool[] bitField)
    {
        ...
        this.payloadLength = (int)Math.Ceiling(bitField.Length / (decimal)8);
    ...
    public BitFieldMessage(long pieceCount, IEnumerable<int> missingPieces)
    {
        ...
        for (var i = 0; i < pieceCount; i++) this.BitField[i] = true;

        foreach (var missingPiece in missingPieces)
            if (missingPiece < pieceCount)
                this.BitField[missingPiece] = false;

        this.payloadLength = (int)Math.Ceiling(this.BitField.Length / (decimal)8);
        this.messageLength = MessageIdLength + this.payloadLength;
    }
    ...
    {
        get { return MessageLengthLength + MessageIdLength + this.payloadLength; }
    }
    ...
    /// </returns>
    public static bool TryDecode(byte[] buffer, ref int offsetFrom, int offsetTo, out BitFieldMessage message,
    ...
    {
        var byteField = new byte[this.payloadLength];
        var written = offset;

        new BitArray(this.BitField).CopyTo(byteField, 0);

        Write(buffer, ref written, this.messageLength);
    ...
    public override string ToString()
    {
        ...
        for (var i = 0; i < this.BitField.Length; i++) sb.Append(this.BitField[i] ? "1" : "0");
