    ...
    public static string ToPeerId(byte[] value)
    {
        ...
        if (delimiterIndex > 0)
            peerId = ReadString(value, ref offset, delimiterIndex + 1) + // client id
                     ReadBytes(value, ref offset, value.Length - delimiterIndex - 1)
                         .ToHexaDecimalString(); // random number
        else
            peerId = ReadBytes(value, ref offset, value.Length)
                .ToHexaDecimalString(); // could not interpret peer id -> read as binary string
