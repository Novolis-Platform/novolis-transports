...
public class RequestMessage : PeerMessage
{
    ...
    {
        get { return MessageLengthLength + MessageIdLength + PieceIndexLength + BlockOffsetLength + BlockLengthLength; }
    }
    ...
    /// </returns>
    public static bool TryDecode(byte[] buffer, ref int offsetFrom, int offsetTo, out RequestMessage message,
        out bool isIncomplete)
    {
        ...
        if (buffer != null &&
            buffer.Length >= offsetFrom + MessageLengthLength + MessageIdLength + PieceIndexLength + BlockOffsetLength +
            BlockLengthLength &&
            offsetFrom >= 0 &&
            offsetTo >= offsetFrom &&
    ...
    public override int Encode(byte[] buffer, int offset)
    {
        ...
        Write(buffer, ref written, this.BlockLength);
