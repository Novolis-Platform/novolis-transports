...
public sealed class Peer : IDisposable
{
    ...
    private void Download()
    {
        ...
        if (!this.isDownloading)
        {
            ...
            {
                if (this.pieceManager.IsComplete) break;

                // process messages
                foreach (var message in this.DequeueDownloadMessages())
                ...
                if (this.HandshakeState == HandshakeState.SendAndReceived)
                {
                            ...
                            if (++unchokeMessagesSent > 10)
                                this.OnCommunicationErrorOccurred(this,
                                    new PeerCommunicationErrorEventArgs(
                                        $"Choked for more than {TimeSpan.FromSeconds(chokeTimeout.TotalSeconds * 10)}.",
                                        true));
                    ...
                    else if (this.SeedingState == SeedingState.Unchoked)
                    {
                        if (piece == null)
                        {
                            ...
                            for (var pieceIndex = 0; pieceIndex < this.BitField.Length; pieceIndex++)
                                if (this.pieceManager.BitField[pieceIndex] == PieceStatus.Missing)
                                    if (this.BitField[pieceIndex] ||
                                        this.pieceManager.IsEndGame)
                                    {
                                        pieceData = pieceData.Length == this.pieceManager.GetPieceLength(pieceIndex)
                                            ? pieceData
                                            : new byte[this.pieceManager.GetPieceLength(pieceIndex)];
                                        bitFieldData =
                                            bitFieldData.Length == this.pieceManager.GetBlockCount(pieceIndex)
                                                ? bitFieldData
                                                : new bool[this.pieceManager.GetBlockCount(pieceIndex)];
                            ...
                            if (piece != null)
                                ...
                                for (var i = 0; i < piece.BitField.Length; i++)
                                    if (!piece.BitField[i])
                                        this.EnqueueSendMessage(new RequestMessage(piece.PieceIndex,
    ...
    private void OnCommunicationErrorOccurred(object sender, PeerCommunicationErrorEventArgs e)
    {
        ...
        if (this.CommunicationErrorOccurred != null) this.CommunicationErrorOccurred(sender, e);
    }
    ...
    private void ProcessRecievedMessage(HaveMessage? message)
    {
        ...
        if (this.HandshakeState == HandshakeState.SendAndReceived)
        {
            ...
            {
                this.OnCommunicationErrorOccurred(this,
                    new PeerCommunicationErrorEventArgs("Invalid have message.", false));
            }
        }
        else
        {
    ...
    private void UpdateTrafficParameters(long downloaded, long uploaded)
    {
        ...
        lock (this.locker)
        {
            ...
            {
                this.downloadSpeed = this.previouslyDownloaded / (decimal)this.stopwatch.Elapsed.TotalSeconds;
                this.uploadSpeed = this.previouslyUploaded / (decimal)this.stopwatch.Elapsed.TotalSeconds;
    ...
    private void Upload()
    {
        ...
        if (!this.isUploading)
        {
            ...
            while (!this.IsDisposed)
            {
                foreach (var message in this.DequeueUploadMessages())
