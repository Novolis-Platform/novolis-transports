...
public sealed class PersistenceManager : IDisposable
{
    ...
    public PersistenceManager(string directoryPath, long torrentLength, long pieceLength,
        IEnumerable<string> pieceHashes, IEnumerable<TorrentFileInfo> files)
    {
        ...
        foreach (var file in files)
            if (file.Download)
            {
                ...
                this.files.Add(file,
                    new FileStream(Path.Combine(this.DirectoryPath, file.FilePath), FileMode.Open, FileAccess.ReadWrite,
                        FileShare.None));
            }
    }
            ...
            if (length > 0)
            {
                ...
                foreach (var file in this.files)
                {
                        ...
                        pieceIndex <= pieceEnd)
                    {
                        ...
                        if (file.Key.Download) this.Read(file.Value, fileOffset, length, pieceData, pieceOffset);
            ...
            if (length == pieceData.Length)
            {
                ...
                foreach (var file in this.files)
                {
                        ...
                        pieceIndex <= pieceEnd)
                    {
                        ...
                        if (file.Key.Download) this.Write(file.Value, fileOffset, length, pieceData, pieceOffset);
    ...
    {
        var bitField = new PieceStatus[this.pieceHashes.Count()];
        long pieceStart;
        long pieceEnd;
        ...
        long torrentEndOffset;
        long fileOffset;
        var pieceData = new byte[this.pieceLength];
        var pieceOffset = 0;
        var length = 0;
        var ignore = false;
        var download = false;
        ...
        lock (this.locker)
        {
            foreach (var file in this.files)
            {
                ...
                for (var pieceIndex = pieceStart; pieceIndex <= pieceEnd; pieceIndex++)
                {
                    ...
                    {
                        bitField[previousPieceIndex] = this.GetStatus(ignore, download,
                            this.pieceHashes.ElementAt((int)previousPieceIndex),
    ...
    {
        if (this.IsDisposed) throw new ObjectDisposedException("Novolis.Transports.Torrent");
    }
