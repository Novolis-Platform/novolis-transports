...
using Novolis.Transports.Torrent.PeerWireProtocol;
using Novolis.Transports.Torrent.PeerWireProtocol.TorrentEventArgs;
using Novolis.Transports.Torrent.TrackerProtocol;
using Novolis.Transports.Torrent.TrackerProtocol.Http;
using Novolis.Transports.Torrent.TrackerProtocol.TorrentEventArgs;
using Novolis.Transports.Torrent.TrackerProtocol.Udp;
...
public sealed class TransferManager : IDisposable
{
    ...
    public TransferManager(int listeningPort, TorrentInfo torrentInfo, ThrottlingManager throttlingManager,
    ...
    {
        get { return this.pieceManager.CompletedPercentage; }
    }
            ...
            if (!this.peers.ContainsKey(tcp.Client.RemoteEndPoint as IPEndPoint))
            {
                if (this.LeechingPeerCount < maxLeechers)
                {
                    ...
                    tcp.ReceiveBufferSize =
                        Math.Max(this.TorrentInfo.BlockLength, this.TorrentInfo.PieceHashes.Count()) + 100;
    ...
    public void Start()
    {
        ...
        this.pieceManager = new PieceManager(this.TorrentInfo.InfoHash, this.TorrentInfo.Length,
            this.TorrentInfo.PieceHashes, this.TorrentInfo.PieceLength, this.TorrentInfo.BlockLength,
            this.persistenceManager.Verify());
        ...
        lock (((IDictionary)this.trackers).SyncRoot)
        {
            foreach (var tracker in this.trackers.Values) tracker.StartTracking();
        }

        this.OnTorrentStarted(this, EventArgs.Empty);

        if (this.pieceManager.IsComplete) this.OnTorrentSeeding(this, EventArgs.Empty);
    }
    ...
    public void Stop()
    {
        ...
        lock (((IDictionary)this.trackers).SyncRoot)
        {
            foreach (var tracker in this.trackers.Values) tracker.StopTracking();
        }
    ...
    /// <summary>
    ///     Adds an outbound peer connection.
    /// </summary>
    public void AddPeer(IPEndPoint endpoint) => this.AddSeeder(endpoint);
    ...
    private void AddSeeder(IPEndPoint endpoint)
    {
        ...
        lock (((IDictionary)this.peers).SyncRoot)
        {
            if (!this.peers.ContainsKey(endpoint))
            {
                ...
                tcp.Client.ReceiveTimeout = (int)TimeSpan.FromSeconds(60).TotalMilliseconds;
                tcp.Client.SendTimeout = tcp.Client.ReceiveTimeout;
                tcp.BeginConnect(endpoint.Address, endpoint.Port, this.PeerConnected,
                    new AsyncConnectData(endpoint, tcp));
            }
        }
    }
    ...
    private void OnTorrentHashing(object sender, EventArgs e)
    {
        ...
        if (this.TorrentHashing != null) this.TorrentHashing(sender, e);
    }
    ...
    private void OnTorrentLeeching(object sender, EventArgs e)
    {
        ...
        if (this.TorrentLeeching != null) this.TorrentLeeching(sender, e);
    }
    ...
    private void OnTorrentSeeding(object sender, EventArgs e)
    {
        ...
        if (this.TorrentSeeding != null) this.TorrentSeeding(sender, e);
    }
    ...
    private void OnTorrentStarted(object sender, EventArgs e)
    {
        ...
        if (this.TorrentStarted != null) this.TorrentStarted(sender, e);
    }
    ...
    private void OnTorrentStopped(object sender, EventArgs e)
    {
        ...
        if (this.TorrentStopped != null) this.TorrentStopped(sender, e);
    }
    ...
    private void Peer_CommunicationErrorOccurred(object sender, PeerCommunicationErrorEventArgs e)
    {
        ...
        if (e.IsFatal)
        {
            ...
            lock (((IDictionary)this.peers).SyncRoot)
            {
                ...
                if (this.peers.ContainsKey(peer.Endpoint)) this.peers.Remove(peer.Endpoint);
    ...
    {
        Debug.WriteLine(
            $"tracking failed for tracker {e.TrackerUri} for torrent {this.TorrentInfo.InfoHash}: \"{e.FailureReason}\"");
