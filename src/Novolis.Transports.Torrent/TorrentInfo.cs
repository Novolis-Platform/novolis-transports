...
public sealed class TorrentInfo
{
    ...
    private TorrentInfo(BEncodedDictionary dictionary, string infoHash, long pieceLength,
        IEnumerable<string> piecesHashValues, bool isPrivate, IEnumerable<Uri> announceList, DateTime? creationDate,
        string comment, string createdBy, Encoding encoding, IEnumerable<TorrentFileInfo> files)
    {
        ...
        this.BlockLength = PeerMessage.DefaultBlockLength;
        this.BlocksCount = (int)(pieceLength / PeerMessage.DefaultBlockLength);
    }
        ...
        if (value is BEncodedDictionary)
        {
                ...
                general[infoKey] is BEncodedDictionary)
            {
                    ...
                    info[piecesKey].As<BEncodedString>().TextBytes.Length % 20 == 0)
                {
                    ...
                    if (pieceHashes.Count == 0) return false;
                }
                else
                {
                    return false;
                         ...
                         info[filesKey] is BEncodedList)
                {
                    ...
                    foreach (var item in info[filesKey].As<BEncodedList>())
                        ...
                        {
                            filePath = Path.Combine(tmpString,
                                Path.Combine(item.As<BEncodedDictionary>()[pathKey].As<BEncodedList>()
                                    .Select(x => x.As<BEncodedString>().Text).ToArray()));
            ...
            if (general.ContainsKey(creationDateKey) &&
                ...
                creationDate = new DateTime(1970, 1, 1, 0, 0, 0, 0)
                    .AddSeconds(general[creationDateKey].As<BEncodedNumber>().Number).ToLocalTime();
    ...
    public int GetBlockCount(int pieceIndex)
    {
        ...
        return (int)Math.Ceiling(this.GetPieceLength(pieceIndex) / (decimal)this.BlockLength);
    }
    ...
    public int GetEndPieceIndex(string filePath)
    {
        ...
        for (var i = 0; i < this.Files.Count(); i++)
        {
            ...
            pieceEnd = pieceStart + (int)Math.Ceiling(info.Length / (decimal)this.PieceLength) - 1;

            if (info.FilePath == filePath) return pieceEnd;
    ...
    public int GetStartPieceIndex(string filePath)
    {
        ...
        for (var i = 0; i < this.Files.Count(); i++)
        {
            ...
            if (info.FilePath == filePath) return pieceStart;
    ...
    public void Save(string torrentInfoFilePath)
    {
        ...
        if (File.Exists(torrentInfoFilePath)) File.Delete(torrentInfoFilePath);
